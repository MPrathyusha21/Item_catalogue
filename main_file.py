...
@app.route('/BookStore/<int:objid>/AllCompanys')
def showBooks(objid):
    library = session.query(BookYard).all()
...
@app.route('/BookStore/addBookName', methods=['POST', 'GET'])
def addBookName():
...
# Edit Book Category
@app.route('/BookStore/<int:objid>/edit', methods=['POST', 'GET'])
...
@app.route('/BookStore/<int:objid>/delete', methods=['POST', 'GET'])
...
# Add New Book Name Details
@app.route('/BookStore/addCompany/addBookDetails/<string:objname>/add',
           methods=['GET', 'POST'])
def addBookDetails(objname):
...
# Edit Book details
@app.route('/BookStore/<int:objid>/<string:obj3name>/edit',
           methods=['GET', 'POST'])
def editBook(objid, obj3name):
    obj = session.query(BookYard).filter_by(id=objid).one()
...
@app.route('/BookStore/<int:objid>/<string:obj3name>/delete',
           methods=['GET', 'POST'])
def deleteBook(objid, obj3name):
    obj = session.query(BookYard).filter_by(id=objid).one()
    bookdetails = session.query(BookName).filter_by(name=obj3name).one()
    # See if the logged in user is not the owner of book
    creator = getUserInfo(obj.user_id)
...
def allBooksJSON():
    bookcategories = session.query(BookYard).all()
...
@app.route('/BookStore/<path:book_name>/book/JSON')
def categoryItemsJSON(book_name):
    bookCategory = session.query(BookYard).filter_by(name=book_name).one()
    books = session.query(BookName).filter_by(bookname=bookCategory).all()
...
def ItemJSON(book_name, edition_name):
    ...
    bookEdition = session.query(BookName).filter_by(
           name=edition_name, bookname=bookCategory).one()
